% simulate_enzymatic.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enzymatic(tout,theta)
% [...] = simulate_enzymatic(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_enzymatic(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enzymatic(varargin)
...
if(options_ami.sensi<2)
    options_ami.id = transpose([0  0  0  0]);
end
...
if(isempty(options_ami.qpositivex))
    options_ami.qpositivex = zeros(4,1);
else
    if(numel(options_ami.qpositivex)==4)
        options_ami.qpositivex = options_ami.qpositivex(:);
    else
        error('Number of elements in options_ami.qpositivex does not match number of states 4');
    end
end
plist = options_ami.sens_ind-1;
if(nargin>=4)
    if(isempty(varargin{4}));
        data=amidata(length(tout),4,0,options_ami.nmaxevent,length(kappa));
    else
...
if(~isempty(options_ami.sx0))
    ...
    options_ami.sx0 = bsxfun(@times,options_ami.sx0,1./permute(theta(options_ami.sens_ind),[2,1]));
end
sol = ami_enzymatic(tout,theta(1:4),kappa(1:4),options_ami,plist,pbar,xscale,data);
if(options_ami.sensi==1)
